...
def load_faq_data() -> Optional[str]:
    ...
    if not base_faq_data_dir.is_dir():
        print(
            f"FATAL: FAQ data directory not found at {base_faq_data_dir}. Ensure 'data' folder exists."
        )
        return None
    ...
    try:
        ...
        for file_path in all_file_paths:
            ...
            question_content = (
                clean_html(question_history[-1].get("content"))
                if question_history
                else None
            )
            ...
            professor_answer_entry = next(
                (
                    child
                    for child in json_data.get("result", {}).get("children", [])
                    if child.get("type") == "i_answer"
                ),
                None,
            )
            answer_content = (
                clean_html(professor_answer_entry["history"][0].get("content"))
                if professor_answer_entry and professor_answer_entry.get("history")
                else None
            )
